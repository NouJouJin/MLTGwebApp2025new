...
- **イベントチケット専用フォーム** - 名前・メールアドレス・PeatixセミナーURL・セミナー名を入力
- **NFT種別による自動フォーム切り替え** - Key IDに基づき適切なフォームを表示
...
NFT_CONTRACT_ADDRESS=0xe828376750fa25a27d67ba7f5e9908ed85f9738b
NFT_CONTRACT_ADDRESS_2027=0x5ecbE52F8c34888e54393b3A83cc7a838Fe0D417
```
...
2. NFTの種別に応じたフォームが自動表示されます
   ...
   **オリジナルTシャツNFT**
   - 上記に加えて**サイズ**選択（S/M/L/XL/その他）

   **イベントチケットNFT**（Key ID: `recSuNlEXiAhHWJFA`）
   - **名前**: 2文字以上
   - **メールアドレス**: 有効なメール形式
   - **見たい過去のPeatixのセミナーURL**
   - **セミナー名**
...
#### 追加
- ✨ イベントチケットNFT専用の引き換えフォーム（名前・メール・PeatixセミナーURL・セミナー名）
- ✨ NFT Key IDによるフォーム自動切り替え（通常 / Tシャツ / イベントチケット）

#### 変更
- 🔄 NFTコントラクトアドレスを `0x30961b851A8A766014E53955694b3210718066e5` に更新
- 🔒 `airtable-orders` APIの郵便番号・電話番号バリデーションをイベントチケット申請時は任意に変更
